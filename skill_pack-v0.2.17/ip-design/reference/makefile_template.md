...
# Usage: make -f tb/Makefile [target]
#
# Targets:
#   compile_<tc>  - Compile a single testcase (debug use)
#   run_<tc>      - Compile + run a single testcase (debug use)
#   clean         - Remove simulation artifacts
#
# Regression is managed by simforge, not this Makefile.
# Use: simforge submit --tags <tag> -j <n>
...
# ==============================================================
# Regression — managed by simforge
# ==============================================================
# Do NOT add a regression: target that calls make run_* in a loop.
# Regression is handled by simforge:
#   simforge submit --tags regress -j 16
#   simforge status
#   simforge results --filter failed
#
# This Makefile only provides per-case compile/run for manual debugging.
# ==============================================================
# Clean
# ==============================================================
...
Use `SIM ?= vcs` (not `SIM := ...`) so users can override on the command line.
VCS is the default simulator (see `verification_env` skill). The iverilog branch
is kept for environments where VCS is unavailable.
