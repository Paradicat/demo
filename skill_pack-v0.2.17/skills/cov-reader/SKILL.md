...
Before your first `cov_reader` command, run this **once** to isolate your daemons from other agents running on the same machine:

```bash
export AGENT_SESSION_ID=$(python3 -c "import uuid; print(uuid.uuid4().hex[:8])")
```

**Why this matters:** Multiple agents can run concurrently on the same machine. Without `AGENT_SESSION_ID`, all agents share the same default session — one agent's `close --all` would shut down another agent's daemons, or parallel agents opening the same coverage file would interfere with each other. Once exported, all subsequent `cov_reader` commands in this conversation automatically use this session without needing `--session`.
